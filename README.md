...
- Exposes a **minimal browser API** (consumed by the bundled UI): list/create/delete conversations, send messages, list connectors (session resolved scope + per-conversation active state), toggle connectors per conversation. Streaming follows the assistant-ui native protocol (Vercel AI SDK data stream).
