...
| **connector** | A tool or retrieval provider attached to a session. Has `descriptive_id` (**stable semantic identity across sessions** — saved active-state keys on it; see [adr-0010 §Stability](../architecture/adrs/0010-unified-connector-model.md#stability-of-descriptive_id)), `name`, `type` (`mcp` or `rag`), `default_active`, and type-specific config. Provisioned at session creation by the integrator; the end user can enable/disable connectors mid-conversation but cannot add new ones. Only active connectors expose tools to the LLM. See [adr-0010](../architecture/adrs/0010-unified-connector-model.md). |
